...
> **Source**: This skill is adapted from [vercel-labs/agent-browser](https://github.com/vercel-labs/agent-browser/tree/main/skills/agent-browser) and customized for Ralph.
...
> **Important**: Always close your session when finished. Unclosed sessions leave orphaned browser processes (chrome-headless-shell) running in the background.
...
# Close specific sessions when done
agent-browser --session test1 close
agent-browser --session test2 close
```

> **Note**: Each session must be closed separately. Commands without `--session` use the "default" session.
